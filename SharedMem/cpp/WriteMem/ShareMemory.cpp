...
void SHAREDMEMORY::RecieveVectorBox(vector<TrackBox>& VTrackBox) {
	...
	{
		box.push_back(atoi(token.c_str()));
	}

	for (int i = 0; i < box.size() / 5; i++)
	{
		tbox.x = box[i * 5 + 0];
		tbox.y = box[i * 5 + 1];
		tbox.width = box[i * 5 + 2];
		tbox.height = box[i * 5 + 3];
		tbox.track_id = box[i * 5 + 4];
		VTrackBox.push_back(tbox);
	}
}
